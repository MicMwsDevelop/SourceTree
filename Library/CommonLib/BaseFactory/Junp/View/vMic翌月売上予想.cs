...
namespace CommonLib.BaseFactory.Junp.View
{
	public class vMic翌月売上予想
    {
        ...
        /// <returns></returns>
        public static List<vMic翌月売上予想> DataTableToList(DataTable table)
        {
            if (null != table && 0 < table.Rows.Count)
            {
                List<vMic翌月売上予想> result = new List<vMic翌月売上予想>();
                foreach (DataRow row in table.Rows)
                {
                    vMic翌月売上予想 data = new vMic翌月売上予想
                    {

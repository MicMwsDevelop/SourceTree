...
{
	public class HardSubscriptEarningsOut
	{
		/// <summary>
		/// 顧客No
		/// </summary>
		...
		/// デフォルトコンストラクタ
		/// </summary>
		public HardSubscriptEarningsOut()
		{
			顧客No = 0;
			顧客名 = string.Empty;
			得意先コード = string.Empty;
		...
		/// <returns></returns>
		public static List<HardSubscriptEarningsOut> DataTableToList(DataTable table)
		{
			if (null != table && 0 < table.Rows.Count)
			{
				List<HardSubscriptEarningsOut> result = new List<HardSubscriptEarningsOut>();
				foreach (DataRow row in table.Rows)
				{
					HardSubscriptEarningsOut data = new HardSubscriptEarningsOut();

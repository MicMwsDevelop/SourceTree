			...
			string sqlStr = string.Format("SELECT"
								...
								+ " ("
									...
									+ " ("
										...
										+ ", '' AS 商品名2"
										+ " FROM {0} AS NYKD"
										+ " INNER JOIN vMicPCA仕入先マスタ AS RMS ON NYKD.nykd_tcd = RMS.rms_tcd"
										+ " WHERE NYKD.nykd_scd Between '000000' And '999999'"
									+ ") as PCA仕入明細"
									+ " WHERE convert(varchar, RTrim(倉庫コード)) <> '99' AND PCA仕入明細.仕入日 Between {1} And {2} AND PCA仕入明細.科目区分 = 0) as 対象月全仕入れ明細"
									+ " LEFT JOIN"
									+ " ("
										+ " SELECT 商品ｺｰﾄﾞ, 評価単価"
										+ " FROM [charlieDB].[dbo].TEST_在庫一覧表"
		...
		public static List<当月仕入単価> Select_当月仕入単価貯蔵品(YearMonth ym, string connectStr)
		{
			string sqlStr = string.Format("SELECT"
										...
										+ " ("
											...
											+ ") as PCA仕入明細貯蔵品"
											+ " WHERE PCA仕入明細貯蔵品.仕入日 Between {1} AND {2}"
										+ ") as 対象月仕入明細貯蔵品"
										+ " LEFT JOIN"
										+ " ("
											+ " SELECT 商品ｺｰﾄﾞ, 評価単価"
										...
										+ " ("
											...
											+ " ("
												...
												+ " ("
													...
													+ " WHERE NYKD.nykd_scd Between '000000' AND '999999'"
												+ ") as PCA仕入明細"
												+ " WHERE convert(varchar, RTrim(倉庫コード)) <> '99' AND PCA仕入明細.仕入日 Between {1} AND {2} AND PCA仕入明細.科目区分 = 0"
											+ ") as 対象月全仕入れ明細"
		...
		public static List<PCA仕入明細> Select_対象月社内仕入明細(YearMonth ym, string connectStr)
		{
			string sqlStr = string.Format("SELECT DISTINCT"
										...
										+ " ("
											...
											+ " WHERE convert(varchar, RTrim(倉庫コード)) <> '99' AND PCA仕入明細.仕入日 Between {1} AND {2} AND PCA仕入明細.科目区分 = 0"
										+ ") as 対象月全仕入れ明細"
										+ " LEFT JOIN tMih支店情報 ON 対象月全仕入れ明細.倉庫code = tMih支店情報.fPca倉庫コード"

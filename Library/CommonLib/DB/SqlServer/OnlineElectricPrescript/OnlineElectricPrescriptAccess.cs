...
namespace CommonLib.DB.SqlServer.OnlineElectricPrescript
{
	public static class OnlineElectricPrescriptAccess
	{
		//////////////////////////////////////////////////////////////////
		/// CharlieDB
		//////////////////////////////////////////////////////////////////
		...
		public static List<OnlineElectricPrescriptEarningsFileOut> GetElectricPrescriptionEarningsOut(Date applyDate, string connectStr)
		{
			DataTable dt = OnlineElectricPrescriptGetIO.GetOnlineElectricPrescriptEarningsOut(applyDate, connectStr);
			return OnlineElectricPrescriptEarningsFileOut.DataTableToList(dt);
		}

		/// <summary>
		/// オン資電子処方箋連携契約情報 売上日時の設定
		/// </summary>
		/// <param name="sale">オン資電子処方箋連携契約情報</param>
		/// <param name="procName">アプリ名</param>
		/// <param name="connectStr">SQL Server接続文字列</param>
		/// <returns>結果行数</returns>
		public static int UpdateSetOnlineElectricPrescriptSaleDate(OnlineElectricPrescriptEarningsFileOut sale, string procName, string connectStr)
		{
			string updateStr = string.Format(@"UPDATE {0} SET [SalesDate] = @1, [UpdateDate] = @2, [UpdatePerson] = @3 WHERE [ApplyNo] = {1}"
								, CharlieDatabaseDefine.TableName[CharlieDatabaseDefine.TableType.T_USE_ELECTRIC_PRESCRIPTION]   // 0
								, sale.受付番号);   // 1
		...
		/// <summary>
		/// 顧客管理利用情報にｵﾝﾗｲﾝ資格確認電子処方箋連携サービスを追加
		/// </summary>
		/// <param name="sale">オン資電子処方箋連携費売上情報</param>
		/// <param name="procName">アプリ名</param>
		/// <param name="connectStr">SQL Server接続文字列</param>
		/// <returns>結果行数</returns>
		public static int SetCustomerUseInformation(OnlineElectricPrescriptEarningsFileOut sale, string procName, string connectStr)
		{
			string whereStr = string.Format("CUSTOMER_ID = {0} AND SERVICE_ID = {1}", sale.顧客No, (int)ServiceCodeDefine.ServiceCode.OnlineLicenseElectricPrescript);
			...
			else
			{
				...
				cui.SERVICE_ID = (int)ServiceCodeDefine.ServiceCode.OnlineLicenseElectricPrescript;

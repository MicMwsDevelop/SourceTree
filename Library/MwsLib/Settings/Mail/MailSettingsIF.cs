...
namespace MwsLib.Settings.Mail
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class MailSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "MailSettings.xml";
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						Settings = serializer.Deserialize(fileStream) as MailSettings;
					}
					catch (Exception)
					{
						result = false;
		...
		public static MailSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as MailSettings;
		}
		...
		public static void SetSettings(MailSettings settings)
		{
			SetSettingsFileName();

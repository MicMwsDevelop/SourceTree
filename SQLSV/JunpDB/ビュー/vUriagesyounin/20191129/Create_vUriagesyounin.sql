...
/****** Object:  View [dbo].[vUriagesyounin]    Script Date: 2019/11/29 14:21:07 ******/
SET ANSI_NULLS ON
GO
...
, H.f担当支店名 AS 拠点名
, iif(D.f商品コード = '800121', 'ES', choose(H.f販売種別+1, 'その他', 'VP', 'UG', '月額課金', 'まとめ', 'PC安心')) AS 販売種別
, TA.fBshName2 AS エリア
, H.f担当者名 AS 担当者
...
, H.f受注承認日 AS 受注承認日
, iif(D.f区分 = 207, '×', iif(D.f商品コード is not null, '○', '×')) AS システム伝票
...
LEFT JOIN JunpDB.dbo.tMih受注詳細 AS D ON H.f受注番号 = D.f受注番号 AND (D.f区分 = 1 OR D.f区分 = 202 OR D.f区分 = 204 OR D.f区分 = 207)

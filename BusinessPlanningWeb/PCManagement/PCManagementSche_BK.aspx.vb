...
Partial Class schedule_Scheduleselect
    ...
    Protected Sub B_新規登録_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles B_新規登録.Click
        ...
        For i = 0 To GridView1.Rows.Count - 1
            ...
            SyuRyoF = GridView1.Rows(i).FindControl("CheckBox1")
        ...
        If (checkHard() = True) Then
            lblError.Text = "ハード登録が、他の案件と重複しています。" & lblError.Text
            Exit Sub
        End If
        ...
        For i = 0 To GridView1.Rows.Count - 1
            ...
            If SyuRyoF.Checked = True Then
                ...
                meisyo = GridView1.Rows(i).FindControl("T_名称")
                HCode = GridView1.Rows(i).Cells(1).Text
                H種別 = GridView1.Rows(i).Cells(2).Text

                'ハードコード
                '種別

                'レコード追加
                SQL.CommandText = "INSERT INTO [PCManagement_Details] ([案件No],[案件名],[ハードCode],[ハード名],[ハード種別]) VALUES ('" & 案件No & "','" & T_案件名.Text & "','" & HCode & "', '" & meisyo.Text & "', '" & H種別 & "');"
        ...
        While (startDate <> EndDate)
            ...
            If dsUser.Tables(0).Rows.Count > 0 Then
                ...
                For i = 0 To dsUser.Tables(0).Rows.Count - 1
                    ...
                    案件No = dsUser.Tables(0).Rows(i).Item(0)
                    ...
                    For m = 0 To GridView1.Rows.Count - 1
                        ...
                        SyuRyoF = GridView1.Rows(m).FindControl("CheckBox1")

                        If SyuRyoF.Checked = True Then

                            meisyo = GridView1.Rows(m).FindControl("T_名称")

                            For n = 0 To dsUser2.Tables(0).Rows.Count - 1


                                If meisyo.Text = dsUser2.Tables(0).Rows(n).Item(4) Then

                                    checkHard = True
                                    lblError.Text = "【" & startDate & " " & meisyo.Text & "】"
                                    GoTo LBL_EXIT

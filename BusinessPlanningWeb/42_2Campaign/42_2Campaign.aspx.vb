...
Partial Class Distributor_Distributor
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '抽出文字列作成
        'sqlstr = "SELECT * FROM dbo.vキャンペーン42_0 ORDER BY 本数 DESC"
        sqlstr2 = "SELECT 支店名,目標数,エントリーライン,本数,ポイント,順序 FROM dbo.vキャンペーン42_3 ORDER BY 順序"
        sqlstr = "SELECT 支店名,目標数,エントリーライン,本数,ポイント,順序 FROM dbo.vキャンペーン42_四半期 ORDER BY 順序"
        sqlstr3 = "SELECT 支店名,目標数,エントリーライン,本数,ポイント,順序 FROM dbo.vキャンペーン42_4 ORDER BY 順序"
        sqlstr4 = "SELECT 支店名,目標数,エントリーライン,本数,ポイント,順序 FROM dbo.vキャンペーン42_5 ORDER BY 順序"
        ...
        SqlDataSource3.ConnectionString = "server='SQLSV'; user id='ww_reader'; password='20150801'; database='SalesDB'"
        SqlDataSource3.SelectCommand = sqlstr3

        SqlDataSource4.ConnectionString = "server='SQLSV'; user id='ww_reader'; password='20150801'; database='SalesDB'"
        SqlDataSource4.SelectCommand = sqlstr4
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        'sqlstr2 = "SELECT 支店名,42_月目標数,42_月エントリーライン,本数,ポイント,順序 FROM dbo.vキャンペーン42_3 ORDER BY 順序"
        ...
        'sqlstr2 = "SELECT 支店名,目標数,エントリーライン,本数,ポイント,順序 FROM dbo.vキャンペーン42_2 ORDER BY 順序"

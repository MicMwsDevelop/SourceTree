  ...
  'Ver1.02(2025/07/03 勝呂):2025/08/01の組織変更対応に伴い改修。当分の間は業務に支障があるため、東日本営業部と西日本営業部は残置する
	...
	Protected Sub button集計実行_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles button集計実行.Click
		...
		If dropList集計単位.SelectedIndex = 0 Then
			...
			sqlStr = selectStr & whereStr & termStr & " AND [営業部コード] = '047'" & groupStr & orderStr
			...
			sqlStr = selectStr & whereStr & termStr & " AND [営業部コード] = '048'" & groupStr & orderStr
	...
	Protected Sub GridViewプロモーション営業部_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridViewプロモーション営業部.RowDataBound
		...
		ElseIf e.Row.RowType = DataControlRowType.Footer Then
			...
			' フッターに、累計を表示します。
			e.Row.Cells(3).Text = totalCountプロモーション営業部.ToString("G")
			e.Row.Cells(4).Text = totalPriceプロモーション営業部.ToString("c")
	...
	Protected Sub GridViewソリューション営業部_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridViewソリューション営業部.RowDataBound
		...
		ElseIf e.Row.RowType = DataControlRowType.Footer Then
			e.Row.Cells(0).Text = "合計："
			' フッターに、累計を表示します。
			e.Row.Cells(3).Text = totalCountソリューション営業部.ToString("G")
			e.Row.Cells(4).Text = totalPriceソリューション営業部.ToString("c")
			e.Row.Cells(1).HorizontalAlign = HorizontalAlign.Right
			e.Row.Cells(2).HorizontalAlign = HorizontalAlign.Right
			e.Row.Font.Bold = True
		End If

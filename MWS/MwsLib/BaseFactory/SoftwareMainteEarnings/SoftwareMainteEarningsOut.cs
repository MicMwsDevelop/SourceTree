...
/*
	...
	public class OrderSlipSoftwareMainte
	{
		...
		public string 摘要名
		{
			get
			{
				if (fSV利用開始年月.HasValue && fSV利用終了年月.HasValue)
				{
					Span span = new Span(fSV利用開始年月.Value.PlusYears(1).First, fSV利用終了年月.Value.PlusYears(1).Last);    // １年後に設定
					return string.Format("{0}年{1}月～{2}年{3}月", span.Start.Year, span.Start.Month, span.End.Year, span.End.Month);
				}
				return string.Empty;
			}
		}
		...
		/// <param name="hanbaisakiCode">販売先コード</param>
		/// <param name="saleDate">売上日 </param>
		/// <param name="goods">palette ES ｿﾌﾄｳｪｱ保守料12ケ月 PCA商品マスタ</param>
		/// <param name="pcaVer">PCAバージョン情報 </param>
		/// <returns>CSV文字列</returns>
		public string ToEarnings(int no, string hanbaisakiCode, Date saleDate, vMicPCA商品マスタ goods, int pcaVer)
		{
			...
			pca.商品コード = f商品コード;// 15:商品コード(13)
			pca.マスター区分 = 0;// 16:マスタ区分
			pca.商品名 = StringUtil.GetSubstringByByte(goods.sms_mei, 36);// 17:品名(36)
			pca.倉庫コード = fPca倉庫コード.Value.ToString();// 19:倉庫コード(6)
			pca.数量 = 1;// 22:数量
			pca.単位 = goods.sms_tani;// 23:単位
			pca.単価 = goods.sms_hyo;// 24:単価
			pca.売上金額 = f提供価格.Value;// 25:売上金額
			pca.原単価 = goods.sms_gen;// 26:原単価
			pca.原価金額 = goods.sms_gen;// 27:原価額
			pca.税区分 = f税区分.ToInt();// 31:税区分
			pca.税込区分 = f税込区分.ToInt();// 32:税込区分
			pca.標準価格 = f標準価格.Value;// 34:標準価格
			pca.税率 = (f税率.HasValue && f税率.Value > 0) ? f税率.Value / 10 : 0;// 48:税率
			return pca.ToCsvString(pcaVer);
		}
		...
		public string ToArticle1(int no, string hanbaisakiCode, Date saleDate, int pcaVer)
		{
			...
			pca.商品コード = PCA売上明細汎用データ.ArticleCode;// 15:000014(13) 
			pca.マスター区分 = 4;// 16:マスタ区分
			pca.商品名 = string.Format("{0}様分", StringUtil.GetSubstringByByte(fユーザー, 34));// 17:品名 ○○○○様分(36)
			pca.倉庫コード = fPca倉庫コード.Value.ToString();// 19:倉庫コード(6)
			return pca.ToCsvString(pcaVer);
		}
		...
		public string ToArticle2(int no, string hanbaisakiCode, Date saleDate, int pcaVer)
		{
			...
			pca.商品コード = PCA売上明細汎用データ.ArticleCode;// 15:000014(13) 
			pca.マスター区分 = 4;// 16:マスタ区分
			pca.商品名 = string.Format("得意先No.{0}", f得意先コード);// 17:品名 得意先No. XXXXXX(36)
			pca.倉庫コード = fPca倉庫コード.Value.ToString();// 19:倉庫コード(6)
			return pca.ToCsvString(pcaVer);
		}
	}

		...
		public static List<OrderInfo> ConvertOrderInfo(DataTable table)
		{
			...
			if (null != table)
			{
				...
				foreach (DataRow row in table.Rows)
				{
					...
					order.OrderNo = row["受注番号"].ToString().Trim();
					order.CustomerNo = DataBaseValue.ConvObjectToInt(row["顧客No"]);
					order.ClinicName = row["医院名"].ToString().Trim();
					order.GoodsID = row["商品コード"].ToString().Trim();
					order.GoodsName = row["商品名"].ToString().Trim();
					order.PriceStr = row["料金"].ToString().Trim();
					order.BranchID = row["拠店ID"].ToString().Trim();
					order.BranchName = row["拠点名"].ToString().Trim();
					order.SalesmanID = row["担当者ID"].ToString().Trim();
					order.SalesmanName = row["担当者名"].ToString().Trim();
					...
					order.Remark = row["備考"].ToString().Trim();
					result.Add(order);
				}
			}
			return result;

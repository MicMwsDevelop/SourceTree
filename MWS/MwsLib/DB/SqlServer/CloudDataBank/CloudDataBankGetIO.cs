...
namespace MwsLib.DB.SqlServer.CloudDataBank
{
	public static class CloudDataBankGetIO
	{
		...
		///// <returns>DataTable</returns>
		//public static DataTable GetCloudDataBankEarningsList(Date date, bool sqlsv2)
		//{
		//	DataTable result = null;
		...
		//			+ " FROM {0} as D"
		//			+ " INNER JOIN [charlieDB].[dbo].M_クラウドデータバンク商品 AS G ON D.sykd_scd = G.商品コード"
		//			+ " INNER JOIN {1} AS M ON G.仕入商品コード = M.sms_scd"
		//			+ " WHERE sykd_kingaku <> 0 AND sykd_uribi >= {2} AND sykd_uribi <= {3}"

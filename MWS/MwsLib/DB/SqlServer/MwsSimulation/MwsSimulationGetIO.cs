...
namespace MwsLib.DB.SqlServer.MwsSimulation
{
	public static class MwsSimulationGetIO
	{
		...
		public static DataTable GetServiceInfo()
		{
			DataTable result = null;
			using (SqlConnection con = new SqlConnection(DataBaseAccess.CreateCharlieConnectionString()))
			{
				try
				{
					// 接続
		...
		public static DataTable GetSetPlan()
		{
			DataTable result = null;
			using (SqlConnection con = new SqlConnection(DataBaseAccess.CreateCharlieConnectionString()))
			{
				try
				{
					...
					string strSQL = @"SELECT GOODS.GOODS_ID AS GoodsID
									...
									LEFT JOIN dbo.M_CODE AS MCODE
										ON MCODE.GOODS_ID = GOODS.GOODS_ID
									LEFT JOIN dbo.M_SERVICE AS MSERVICE
										ON MSERVICE.SERVICE_ID = MCODE.SERVICE_ID
		...
		/// </summary>
		/// <returns>セット割サービス情報の取得レコード数</returns>
		public static DataTable GetSetPlanElement()
		{
			DataTable result = null;
			using (SqlConnection con = new SqlConnection(DataBaseAccess.CreateCharlieConnectionString()))
			{
				try
				{
					...
					string strSQL = @"SELECT SERVICE_ID AS ServiceCode
									, GOODS_ID AS GoodsID
									FROM dbo.M_CODE
									WHERE SET_SALE = 1
			...
			using (SqlConnection con = new SqlConnection(DataBaseAccess.CreateCouplerConnectionString()))
			{
				try
				{
					...
					string strSQL = @"SELECT MSET.SET_ID AS GroupID
									...
									LEFT JOIN dbo.M_SET_SERVICE AS SET_SERVICE

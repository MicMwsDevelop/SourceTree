...
namespace MwsLib.DB.SQLite.ScanImageData
{
	public static class SQLiteScanImageDataSetIO
	{
		...
		/// <param name="list">インデックスファイル情報リスト</param>
		/// <returns>影響行数</returns>
		public static int InsertIntoIndexFileInfo(string dbPath, List<IndexFileInfo> list)
		{
			int rowCount = -1;
			using (SQLiteConnection con = new SQLiteConnection(SQLiteAccess.CreateConnectionString(Path.Combine(dbPath, ScanImageDataDef.SCAN_IMAGE_DATA_DATABASE_NAME))))
			{
				try
				{
					...
					using (SQLiteTransaction tran = con.BeginTransaction(IsolationLevel.Serializable))
					{
						try
						{
							string sqlString = string.Format(@"INSERT INTO {0} VALUES (@1, @2, @3)", ScanImageDataDef.INDEX_FILE_TABLE_NAME);
							foreach (IndexFileInfo info in list)
							{
								SQLiteParameter[] param = { new SQLiteParameter("@1", info.TokuisakiNo),	// TOKUISAKI_NO
												new SQLiteParameter("@2", info.CustomerNo),					// CUSTOMER_NO
												new SQLiteParameter("@3", info.ClinicName) };				// CLINIC_NAME

								// 実行
								rowCount = SQLiteController.SqlExecuteCommand(con, tran, sqlString, param);
								if (rowCount <= -1)
								{
									throw new ApplicationException("InsertIntoIndexFileInfo() Error!");
								}
							}
							// コミット
							tran.Commit();
			...
			using (SQLiteConnection con = new SQLiteConnection(SQLiteAccess.CreateConnectionString(Path.Combine(dbPath, ScanImageDataDef.SCAN_IMAGE_DATA_DATABASE_NAME))))
			{
				try
				{
					...
					using (SQLiteTransaction tran = con.BeginTransaction(IsolationLevel.Serializable))
					{
						try
						{
							string sqlString = string.Format(@"DELETE FROM {0}", ScanImageDataDef.INDEX_FILE_TABLE_NAME);

							// 実行
							rowCount = SQLiteController.SqlExecuteCommand(con, tran, sqlString);
							if (rowCount <= -1)
							{
								throw new ApplicationException("DeleteIndexFileInfo() Error!");
							}
							// コミット
							tran.Commit();
						}

...
namespace NarcohmOrderCheck.Mail
{
	/// <summary>
	/// メール送信クラス
	/// </summary>
	public static class SendMailControl
	{
		...
		/// サービス開始連絡メールの送信
		/// </summary>
		/// <param name="narcohm">サービス開始連絡メール情報</param>
		/// <param name="mailAddress">医院名</param>
		public static void SendServiceStartMail(T_NARCOHM_APPLICATE_HEADER narcohm, string mailAddress)
		{
			using (MailMessage msg = new MailMessage())
			{
				...
				// 件名
				msg.Subject = string.Format("{0}開始  WW伝票起票依頼/{1}) {2}", narcohm.DetailList[0].GoodsName, narcohm.CustomerNo, narcohm.ClinicName);
				...
				using (var sr = new StreamReader(Path.Combine(Directory.GetCurrentDirectory(), conf["body_start"]), enc))
				{
					...
					// パラメータ（%～%）の置換
					msg.Body = sr.ReadToEnd().Replace("%CUSTOMER_NO%", narcohm.CustomerNo.ToString()).Replace("%CLINIC_NAME%", narcohm.ClinicName).Replace("%SERVICE_START_DATE%", narcohm.ServiceStartDate.ToString()).Replace("%MESSAGE%", body);
				}
				// メール送信
				SendMailControl.SendMail(msg, conf, mailAddress);
			}
		}
		...
		///// <param name="mailList">送信メール情報</param>
		///// <param name="pcList">PC安心サポート管理情報</param>
		//public static void SendBranchMail(NarcohmApplicate narcohm, string mailAddress)
		//{
		//	using (MailMessage msg = new MailMessage())
		//	{
		//		Encoding enc = Encoding.GetEncoding("shift_jis");
		...
		//		// メール送信
		//		SendMailControl.SendMail(msg, conf, mailAddress.MailAddress, conf["to"]);
		//	}
		//}

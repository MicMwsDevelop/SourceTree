//
// MwsServiceCancelToolSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace MwsServiceCancelTool.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class MwsServiceCancelToolSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "MwsServiceCancelToolSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static MwsServiceCancelToolSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(MwsServiceCancelToolSettings));
						Settings = serializer.Deserialize(fileStream) as MwsServiceCancelToolSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new MwsServiceCancelToolSettings();
				}
			}
			return result;
		}
		...
		public static MwsServiceCancelToolSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as MwsServiceCancelToolSettings;
		}
		...
		public static void SetSettings(MwsServiceCancelToolSettings settings)
		{
			SetSettingsFileName();

//
// PcSupportManagerSettingsIF.cs
// 
// PC安心サポート管理 環境設定インターフェイス
// 
...
namespace PcSupportManager.Settings
{
	...
	/// </summary>
	public static class PcSupportManagerSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILE_NAME = "PcSupportManagerSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static PcSupportManagerSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(PcSupportManagerSettings));
						Settings = serializer.Deserialize(fileStream) as PcSupportManagerSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new PcSupportManagerSettings();
				}
			}
			return result;
		}
		...
		/// </summary>
		/// <returns>true:成功 false:失敗</returns>
		private static bool SavePcSupportManagerSettings()
		{
			bool result = true;
		...
		public static PcSupportManagerSettings GetPcSupportManagerSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as PcSupportManagerSettings;
		}
		...
		public static void SetPcSupportManagerSettings(PcSupportManagerSettings settings)
		{
			SetSettingsFileName();

			Settings = (PcSupportManagerSettings)settings.Clone();

			SavePcSupportManagerSettings();
		}
	}
}

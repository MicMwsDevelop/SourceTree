//
// PriceUpEarningsFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace PriceUpEarningsFile.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class PriceUpEarningsFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "PriceUpEarningsFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static PriceUpEarningsFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(PriceUpEarningsFileSettings));
						Settings = serializer.Deserialize(fileStream) as PriceUpEarningsFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new PriceUpEarningsFileSettings();
				}
			}
			return result;
		}
		...
		public static PriceUpEarningsFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as PriceUpEarningsFileSettings;
		}
		...
		public static void SetSettings(PriceUpEarningsFileSettings settings)
		{
			SetSettingsFileName();

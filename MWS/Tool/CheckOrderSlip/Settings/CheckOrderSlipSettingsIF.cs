...
namespace CheckOrderSlip.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class CheckOrderSlipSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "CheckOrderSlipSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static CheckOrderSlipSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(CheckOrderSlipSettings));
						Settings = serializer.Deserialize(fileStream) as CheckOrderSlipSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new CheckOrderSlipSettings();
				}
			}
			return result;
		}
		...
		public static CheckOrderSlipSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as CheckOrderSlipSettings;
		}
		...
		public static void SetSettings(CheckOrderSlipSettings settings)
		{
			SetSettingsFileName();

//
// AlmexMaintePurchaseFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace AlmexMaintePurchaseFile.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class AlmexMaintePurchaseFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "AlmexMaintePurchaseFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static AlmexMaintePurchaseFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(AlmexMaintePurchaseFileSettings));
						Settings = serializer.Deserialize(fileStream) as AlmexMaintePurchaseFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new AlmexMaintePurchaseFileSettings();
				}
			}
			return result;
		}
		...
		public static AlmexMaintePurchaseFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as AlmexMaintePurchaseFileSettings;
		}
		...
		public static void SetSettings(AlmexMaintePurchaseFileSettings settings)
		{
			SetSettingsFileName();

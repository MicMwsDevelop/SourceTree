...
namespace MwsCuiEditTool.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class MwsCuiEditToolSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "MwsCuiEditToolSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static MwsCuiEditToolSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(MwsCuiEditToolSettings));
						Settings = serializer.Deserialize(fileStream) as MwsCuiEditToolSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new MwsCuiEditToolSettings();
				}
			}
			return result;
		}
		...
		public static MwsCuiEditToolSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as MwsCuiEditToolSettings;
		}
		...
		public static void SetSettings(MwsCuiEditToolSettings settings)
		{
			SetSettingsFileName();

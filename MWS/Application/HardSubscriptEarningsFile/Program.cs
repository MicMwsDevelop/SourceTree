...
using HardSubscriptEarningsFile.Mail;
...
namespace HardSubscriptEarningsFile
{
	internal static class Program
	{
		...
		/// 環境設定
		/// </summary>
		public static HardSubscriptEarningsFileSettings gSettings;
		...
		static int Main()
		{
			...
			gSettings = HardSubscriptEarningsFileSettingsIF.GetSettings();
			...
			if (2 <= cmds.Length)
			{
				if ("AUTO" == cmds[1].ToUpper())
				{
					...
					HardSubscriptEarningsFileSettingsIF.SetSettings(gSettings);
					if (0 < msg.Length)
					{
						return 1;
					}
		...
		public static string OutputCsvFile(Date bootDate)
		{
			...
			try
			{
				...
				List<HardSubscriptEarningsOut> saleList = HardSubscriptAccess.GetHardSubscriptEarningsOut(firstDate, gSettings.ConnectCharlie.ConnectionString);
				if (null != saleList && 0 < saleList.Count)
				{
					...
					{
						foreach (HardSubscriptEarningsOut sale in saleList)
						{
...
#if !DebugNoWrite
						// ハードサブスク管理契約情報 課金終了日、終了フラグの設定
						HardSubscriptAccess.UpdateSetHardSubscriptHeader(sale, serviceEndFlag, gSettings.ConnectCharlie.ConnectionString, PROC_NAME);
#endif
					}
				}
				else

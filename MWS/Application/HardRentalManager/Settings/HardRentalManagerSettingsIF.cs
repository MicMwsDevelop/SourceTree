//
// HardRentalManagerSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace HardRentalManager.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class HardRentalManagerSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "HardRentalManagerSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static HardRentalManagerSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(HardRentalManagerSettings));
						Settings = serializer.Deserialize(fileStream) as HardRentalManagerSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new HardRentalManagerSettings();
				}
			}
			return result;
		}
		...
		public static HardRentalManagerSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as HardRentalManagerSettings;
		}
		...
		public static void SetSettings(HardRentalManagerSettings settings)
		{
			SetSettingsFileName();

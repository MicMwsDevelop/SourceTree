...
namespace HardRentalManager.Mail
{
	/// <summary>
	/// メール送信クラス
	/// </summary>
	public static class SendMailControl
	{
		...
		public static void SendMailToGyomu(Date useNotifyDate, List<HardRentalNotify> notifyList)
		{
			using (MailMessage msg = new MailMessage())
			{
				...
				// 件名
				msg.Subject = string.Format("{0}にハードレンタルの契約満了を迎える対象ユーザー連絡", ymStr);

				// 本文
				msg.Body += string.Format("<div>"
							+ @"<p>業務管理部 業務課各位</p>"
							+ @"<p>{0}にハードレンタルの契約満了を迎えるユーザーをご連絡します。<br>"
							+ @"<br>"
							+ @"<br>"
							+ @"<p>{0} 契約満了対象ユーザー<br>"
							+ @"</div>", ymStr);
				if (null != notifyList)
				{
					msg.Body += @"<table style=""BORDER-COLLAPSE: collapse"" bordercolor=""black"" border=1>"
								...
								+ @"</tr>";
					foreach (HardRentalNotify notify in notifyList)
					{
				...
				else
				{
					msg.Body += @"<table style=""BORDER-COLLAPSE: collapse"" bordercolor=""black"" border=1>"
								...
								+ @"</tr>"
								+ @"</table>";
					msg.Body += string.Format(@"<br><p>{0}にハードレンタルの契約満了を迎えるユーザーはいませんでした。</p>", ymStr);
				}
				msg.Body += @"</div>"
							+ @"<div>"
		...
		public static void SendMailToOffice(Date useNotifyDate, List<HardRentalNotify> notifyList, tMih支店情報 branch)
		{
			using (MailMessage msg = new MailMessage())
			{
				Encoding enc = Encoding.GetEncoding("shift_jis");
				...
				// 件名
				msg.Subject = string.Format("【{0}】 {1}にハードレンタルの契約満了を迎える対象ユーザー連絡", branch.f支店名, ymStr);

				// 本文
				msg.Body += string.Format("<div>"
							+ @"<p>{0}</p>"
							+ @"<p>{1}にハードレンタルの契約満了を迎えるユーザーをご連絡します。<br>"
							+ @"<br>"
							+ @"<br>"
							+ @"<p>{1} 契約満了対象ユーザー<br>"

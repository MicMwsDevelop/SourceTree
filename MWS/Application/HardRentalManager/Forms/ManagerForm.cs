...
namespace HardRentalManager
{
	...
	public partial class ManagerForm : Form
	{
		...
		private void MainForm_Load(object sender, EventArgs e)
		{
			...
			// ログインユーザー情報の取得
			tUser user = HardRentalAccess.GetLoginUser(Environment.UserName, Program.gSettings.ConnectJunp.ConnectionString);
			if (null != user)
			{
		...
		private void buttonSearch_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				vMic全ユーザー2 user = HardRentalAccess.GetClinicInfo(customerNo, Program.gSettings.ConnectJunp.ConnectionString);
				if (null != user)
				{
					// 顧客情報の設定
		...
		private void buttonModify_Click(object sender, EventArgs e)
		{
			if (0 < listViewHeader.SelectedIndices.Count)
			{
				...
				if (null != lvItem.Tag)
				{
					T_HARD_RENTAL_HEADER header = (T_HARD_RENTAL_HEADER)lvItem.Tag;
					using (HeaderDetailForm form = new HeaderDetailForm())
					{
				...
				if (null != lvItem.Tag)
				{
					...
					if (header.IsDelete)
					{
						if (DialogResult.Yes == MessageBox.Show("契約情報を削除してよろしいですか？", Program.ProgramName, MessageBoxButtons.YesNo, MessageBoxIcon.Question))
						{
							try
							{
								List<T_HARD_RENTAL_DETAIL> detailList = HardRentalAccess.GetHardRentalDetailList(header.InternalRentalNo, Program.gSettings.ConnectCharlie.ConnectionString);
								...
								// 契約情報の削除
								HardRentalAccess.DeleteHardRentalHeader(header.InternalRentalNo, Program.gSettings.ConnectCharlie.ConnectionString);
		...
		private void SetHeaderListView(int customerID)
		{
			try
			{
				...
				// 契約情報リストの取得
				List<T_HARD_RENTAL_HEADER> headerList = HardRentalAccess.GetHardRentalHeaderList(customerID, Program.gSettings.ConnectCharlie.ConnectionString);
				if (null != headerList && 0 < headerList.Count)
				{
					foreach (T_HARD_RENTAL_HEADER header in headerList)
					{

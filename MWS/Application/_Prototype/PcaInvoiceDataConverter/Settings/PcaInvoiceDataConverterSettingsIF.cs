//
// PcaInvoiceDataConverterSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace PcaInvoiceDataConverter.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class PcaInvoiceDataConverterSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "PcaInvoiceDataConverterSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static PcaInvoiceDataConverterSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(PcaInvoiceDataConverterSettings));
						Settings = serializer.Deserialize(fileStream) as PcaInvoiceDataConverterSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new PcaInvoiceDataConverterSettings();
				}
			}
			return result;
		}
		...
		public static PcaInvoiceDataConverterSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as PcaInvoiceDataConverterSettings;
		}
		...
		public static void SetSettings(PcaInvoiceDataConverterSettings settings)
		{
			SetSettingsFileName();

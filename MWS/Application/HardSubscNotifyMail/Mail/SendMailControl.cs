...
namespace HardSubscNotifyMail.Mail
{
	/// <summary>
	/// メール送信クラス
	/// </summary>
	public static class SendMailControl
	{
		...
		/// <param name="notifyList">ハードサブスク通知医院リスト</param>
		public static void SendLimitMailToGyomu(Date endDate, List<HardSubscNotify> notifyList)
		{
			using (MailMessage msg = new MailMessage())
			{
				...
				string ymStr = endDate.ToYearMonth().GetNormalString(); // yyyy/MM

				// 件名
				msg.Subject = string.Format("半年後({0})にハードサブスクの契約満了を迎える対象ユーザー連絡", ymStr);

				// 本文
				msg.Body += string.Format("<div>"
							+ @"<p>業務管理部 業務課各位</p>"
							+ @"<p>半年後({0})にハードサブスクの契約満了を迎えるユーザーをご連絡します。<br>"
							+ @"<br>"
							+ @"<br>"
							+ @"<p>対象ユーザー<br>"
							+ @"</div>", ymStr);
				...
				else
				{
					msg.Body += @"<table style=""BORDER-COLLAPSE: collapse"" bordercolor=""black"" border=1>"
								...
								+ @"</tr>"
								+ @"</table>";
					msg.Body += string.Format(@"<br><p>半年後({0})にハードサブスクの契約満了を迎えるユーザーはいませんでした。</p>", ymStr);
				}
				msg.Body += @"</div>"
							+ @"<div>"
		...
		public static void SendLimitMailToOffice(Date endDate, List<HardSubscNotify> notifyList, tMih支店情報 branch)
		{
			using (MailMessage msg = new MailMessage())
			{
				Encoding enc = Encoding.GetEncoding("shift_jis");
				...
				// 件名
				msg.Subject = string.Format("【{0}】 半年後({1})にハードサブスクの契約満了を迎える対象ユーザー連絡", branch.f支店名, ymStr);

				// 本文
				msg.Body += string.Format("<div>"
							+ @"<p>{0}</p>"
							+ @"<p>半年後({1})にハードサブスクの契約満了を迎えるユーザーをご連絡します。<br>"
							+ @"<br>"
							+ @"<br>"
							+ @"<p>対象ユーザー<br>"
		...
		/// </summary>
		/// <param name="notifyList">ハードサブスク通知医院リスト</param>
		public static void SendFinishedMailToGyomu(List<HardSubscNotify> notifyList)
		{
			using (MailMessage msg = new MailMessage())
			{
				Encoding enc = Encoding.GetEncoding("shift_jis");
				...
				// 件名
				msg.Subject = "当月にハードサブスクの契約を満了する対象ユーザー連絡";

				// 本文
				msg.Body += "<div>"
							+ @"<p>業務管理部 業務課各位</p>"
							+ @"<p>当月にハードサブスクの契約を満了するユーザーをご連絡します。<br>"
							+ @"<br>"
							+ @"<br>"
							+ @"<p>対象ユーザー<br>"
							+ @"</div>";
				...
				else
				{
					msg.Body += @"<table style=""BORDER-COLLAPSE: collapse"" bordercolor=""black"" border=1>"
								...
								+ @"</tr>"
								+ @"</table>";
					msg.Body += @"<br><p>当月にハードサブスクの契約を満了するユーザーはいませんでした。</p>";
				}
				msg.Body += @"</div>"
							+ @"<div>"
							+ @"<p>以上、よろしくお願いいたします。<br>システム管理部</p>"
		...
		/// <param name="notifyList">ハードサブスク通知医院リスト</param>
		/// <param name="branch">支店情報</param>
		public static void SendFinishedMailToOffice(List<HardSubscNotify> notifyList, tMih支店情報 branch)
		{
			using (MailMessage msg = new MailMessage())
			{
				Encoding enc = Encoding.GetEncoding("shift_jis");

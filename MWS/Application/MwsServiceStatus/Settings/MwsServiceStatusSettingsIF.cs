...
namespace MwsServiceStatus.Settings
{
	/// <summary>
	/// 環境設定インターフェイス
	/// </summary>
	public static class MwsServiceStatusSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "MwsServiceStatusSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static MwsServiceStatusSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(MwsServiceStatusSettings));
						Settings = serializer.Deserialize(fileStream) as MwsServiceStatusSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new MwsServiceStatusSettings();
				}
			}
			return result;
		}
		...
		public static MwsServiceStatusSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as MwsServiceStatusSettings;
		}
		...
		public static void SetSettings(MwsServiceStatusSettings settings)
		{
			SetSettingsFileName();

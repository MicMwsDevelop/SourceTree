//
// PcSupportEarningsFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace PcSupportEarningsFile.Settings
{
	/// <summary>
	/// 環境設定インターフェイス
	/// </summary>
	public static class PcSupportEarningsFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "PcSupportEarningsFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static PcSupportEarningsFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(PcSupportEarningsFileSettings));
						Settings = serializer.Deserialize(fileStream) as PcSupportEarningsFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new PcSupportEarningsFileSettings();
				}
			}
			return result;
		}
		...
		public static PcSupportEarningsFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as PcSupportEarningsFileSettings;
		}
		...
		public static void SetSettings(PcSupportEarningsFileSettings settings)
		{
			SetSettingsFileName();

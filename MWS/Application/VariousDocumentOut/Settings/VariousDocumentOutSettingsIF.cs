//
// VariousDocumentOutSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace VariousDocumentOut.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class VariousDocumentOutSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "VariousDocumentOutSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static VariousDocumentOutSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(VariousDocumentOutSettings));
						Settings = serializer.Deserialize(fileStream) as VariousDocumentOutSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new VariousDocumentOutSettings();
				}
			}
			return result;
		}
		...
		public static VariousDocumentOutSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as VariousDocumentOutSettings;
		}
		...
		public static void SetSettings(VariousDocumentOutSettings settings)
		{
			SetSettingsFileName();

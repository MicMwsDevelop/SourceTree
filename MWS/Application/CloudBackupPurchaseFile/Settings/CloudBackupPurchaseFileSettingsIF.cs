//
// CloudBackupPurchaseFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace CloudBackupPurchaseFile.Settings
{
	...
	/// </summary>
	public static class CloudBackupPurchaseFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "CloudBackupPurchaseFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static CloudBackupPurchaseFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(CloudBackupPurchaseFileSettings));
						Settings = serializer.Deserialize(fileStream) as CloudBackupPurchaseFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new CloudBackupPurchaseFileSettings();
				}
			}
			return result;
		}
		...
		public static CloudBackupPurchaseFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as CloudBackupPurchaseFileSettings;
		}
		...
		public static void SetSettings(CloudBackupPurchaseFileSettings settings)
		{
			SetSettingsFileName();

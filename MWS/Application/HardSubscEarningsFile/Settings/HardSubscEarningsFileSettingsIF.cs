//
// HardSubscEarningsFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace HardSubscEarningsFile.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class HardSubscEarningsFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "HardSubscEarningsFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static HardSubscEarningsFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(HardSubscEarningsFileSettings));
						Settings = serializer.Deserialize(fileStream) as HardSubscEarningsFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new HardSubscEarningsFileSettings();
				}
			}
			return result;
		}
		...
		public static HardSubscEarningsFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as HardSubscEarningsFileSettings;
		}
		...
		public static void SetSettings(HardSubscEarningsFileSettings settings)
		{
			SetSettingsFileName();

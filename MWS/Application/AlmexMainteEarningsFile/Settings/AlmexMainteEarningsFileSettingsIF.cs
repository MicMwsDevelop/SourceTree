//
// AlmexMainteEarningsFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace AlmexMainteEarningsFile.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class AlmexMainteEarningsFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "AlmexMainteEarningsFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static AlmexMainteEarningsFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(AlmexMainteEarningsFileSettings));
						Settings = serializer.Deserialize(fileStream) as AlmexMainteEarningsFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new AlmexMainteEarningsFileSettings();
				}
			}
			return result;
		}
		...
		public static AlmexMainteEarningsFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as AlmexMainteEarningsFileSettings;
		}
		...
		public static void SetSettings(AlmexMainteEarningsFileSettings settings)
		{
			SetSettingsFileName();

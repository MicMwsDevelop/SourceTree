...
namespace MakePurchaseFile.Settings
{
	...
	public static class PurchaseTransferSettingsIF
	{
		...
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "MakePurchaseFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static MakePurchaseFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(MakePurchaseFileSettings));
						Settings = serializer.Deserialize(fileStream) as MakePurchaseFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new MakePurchaseFileSettings();
				}
			}
			return result;
		}
		...
		public static MakePurchaseFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as MakePurchaseFileSettings;
		}
		...
		public static void SetSettings(MakePurchaseFileSettings settings)
		{
			SetSettingsFileName();

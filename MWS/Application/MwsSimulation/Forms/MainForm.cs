...
namespace MwsSimulation.Forms
{
	...
	public partial class MainForm : Form
	{
		...
		/// 見積書情報リスト
		/// </summary>
		private List<Estimate> EstimateList { get; set; }
		...
		private void MainForm_Load(object sender, EventArgs e)
		{
			...
			if (0 < listBoxEstimate.Items.Count)
			{
		...
		/// 見積書情報リストボックスの設定
		/// </summary>
		private void SetListBoxEstimate()
		{
			listBoxEstimate.Items.Clear();
		...
		{
			using (SimulationForm form = new SimulationForm())
			{
				if (DialogResult.OK == form.ShowDialog())
				{
					EstimateList.Add(form.EstimateData);

					// 見積書情報リストボックスの設定
					this.SetListBoxEstimate();

					listBoxEstimate.SelectedIndex = listBoxEstimate.Items.Count - 1;
				}
			}
		}
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void listBoxEstimate_MouseDoubleClick(object sender, MouseEventArgs e)
		{
		...
		private void buttonModify_Click(object sender, EventArgs e)
		{
			...
			{
				int saveIndex = listBoxEstimate.SelectedIndex;

				using (SimulationForm form = new SimulationForm(EstimateList[listBoxEstimate.SelectedIndex]))
				{
					if (DialogResult.OK == form.ShowDialog())
					{
						EstimateList[listBoxEstimate.SelectedIndex] = form.EstimateData;

						// 見積書情報リストボックスの設定
						this.SetListBoxEstimate();

						listBoxEstimate.SelectedIndex = saveIndex;
					}
				}
			}
		}
		...
		private void buttonNameChange_Click(object sender, EventArgs e)
		{
			if (-1 != listBoxEstimate.SelectedIndex)
			{
				...
				using (DestinationForm form = new DestinationForm(est.Destination, est.NotUsedMessrs))
				{
					if (DialogResult.OK == form.ShowDialog())
					{
						if (IsExistDestination(est.EstimateID, form.Destination))
						{
		...
		private void buttonPrintEstimate_Click(object sender, EventArgs e)
		{
			if (-1 != listBoxEstimate.SelectedIndex)
			{
				...
				{
					// 見積書印刷
					this.PrintEstimate(PrintEstimateDef.MwsPaperType.Estimate, EstimateList[listBoxEstimate.SelectedIndex], false);
				}
			}
		}
		...
		private void buttonPrintOrder_Click(object sender, EventArgs e)
		{
			if (-1 != listBoxEstimate.SelectedIndex)
			{
				...
				{
					// 注文書印刷
					this.PrintEstimate(PrintEstimateDef.MwsPaperType.PurchaseOrder, EstimateList[listBoxEstimate.SelectedIndex], false);

					if (DialogResult.Yes == MessageBox.Show("注文請書を印刷しますか？", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question))
					{
						// 注文請書印刷
						this.PrintEstimate(PrintEstimateDef.MwsPaperType.OrderConfirm, EstimateList[listBoxEstimate.SelectedIndex], false);
					}
				}
			}
		}
		...
		/// <returns>判定</returns>
		private bool IsExistDestination(int id, string destination)
		{
			if (-1 != EstimateList.FindIndex(p => p.EstimateID != id && p.Destination == destination))
			{
				return true;
			}
			return false;

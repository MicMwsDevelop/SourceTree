//
// SimulationMatomeForm202408.cs
//
// おまとめプラン御見積書作成画面（2024/08～12ヵ月、36ヵ月）
// 
...
{
	/// <summary>
	/// おまとめプラン 御見積書作成画面（2024/08～12ヵ月、36ヵ月）
	/// </summary>
	public partial class SimulationMatomeForm202408 : Form
	{
		/// <summary>
		/// 印刷設定保持用
		/// </summary>
		...
		/// デフォルトコンストラクタ
		/// </summary>
		public SimulationMatomeForm202408()
		{
			InitializeComponent();
		...
		/// コンストラクタ
		/// </summary>
		public SimulationMatomeForm202408(Estimate est)
		{
			InitializeComponent();
		...
		private void SimulationMatomeNewForm_Load(object sender, EventArgs e)
		{
			...
			if (MainForm.gGroupPlanList202408.IsExistKeiyakuMonth(12))
			{
				// おまとめプラン12ヵ月プランが有効
				ExistMatome12 = true;
			...
			if (MainForm.gGroupPlanList202408.IsExistKeiyakuMonth(36))
			{
				// おまとめプラン36ヵ月プランが有効
				ExistMatome36 = true;
		...
		private void buttonPrint_Click(object sender, EventArgs e)
		{
			...
			if (0 < groupList.Count || 0 < serviceList.Count)
			{
				...
				// 申込み種別
				est.Apply = (radioButtonNormal12.Checked || radioButtonNormal36.Checked) ? Estimate.ApplyType.MatomeNone: Estimate.ApplyType.Matome;
		...
		private void buttonOK_Click(object sender, EventArgs e)
		{
			...
			if (0 < groupList.Count || 0 < serviceList.Count)
			{
				...
				if (null == EstimateData)
				{
					...
					// 申込み種別
					EstimateData.Apply = (radioButtonNormal12.Checked || radioButtonNormal36.Checked) ? Estimate.ApplyType.MatomeNone : Estimate.ApplyType.Matome;
		...
		private void DrawServicePrice()
		{
			...
			if (0 == servicePrice)
			{
				...
				labelMatomeMessage.Text = string.Format(@"※あと \{0} でおまとめプラン割引が適用できます。", StringUtil.CommaEdit(MainForm.gServiceList.Platform.Price + MainForm.gMinFreeMonthMinAmmount202408));
			}
			else
			{
				...
				// おまとめプラン契約あり
				GroupPlan plan12 = MainForm.gGroupPlanList202408.GetMachGroupPlan(12, servicePrice);
				GroupPlan plan36 = MainForm.gGroupPlanList202408.GetMachGroupPlan(36, servicePrice);
				...
				{
					labelMatomeMessage.Text = string.Format(@"※あと \{0} でおまとめプラン割引が適用できます。", StringUtil.CommaEdit(MainForm.gMinFreeMonthMinAmmount202408 - servicePrice));
				}
				if (MainForm.gMinAmmount202408 <= servicePrice)
				{
					// おまとめプラン適用金額
				...
				if (radioButtonNormal12.Checked || radioButtonNormal36.Checked)
				{
					// おまとめプラン契約なし
		...
		private void GetOrderService(out List<GroupService> groupList, out List<ServiceInfo> serviceList)
		{
			...
			if (radioButtonMatome12.Checked || radioButtonMatome36.Checked)
			{
				// おまとめプラン契約あり

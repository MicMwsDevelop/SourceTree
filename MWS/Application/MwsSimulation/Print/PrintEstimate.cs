...
namespace MwsSimulation.Print
{
	...
	public class PrintEstimate
	{
		...
		public int SetData(PrintEstimateDef.MwsPaperType type, Estimate est, short taxRate)
		{
			PaperType = type;
			Price = est.GetPrice;
			TaxRate = taxRate;
			Tax = CalcTax.GetTax(taxRate, CalcTax.RoundFraction.Round, Price);		// 四捨五入
				...
				switch (para.GetPrintParaType())
				{
					...
					case PrintParaDef.PrintParaType.Special:
						...
						else if ("<品番" == StringUtil.Left(para.Entry, 3))
						{
							...
							PrintEstimateLine est = PrintData.GetPrintEstimateLine(curPage, line);
							if (null != est)
							{
						...
						else if ("<数量" == StringUtil.Left(para.Entry, 3))
						{
							...
							if (null != est)
							{
								if (false == est.ChildServide)
								{
									...
									if (Estimate.ApplyType.Matome == PrintData.Apply)
									{
										// まとめ契約あり
									...
									{
										// まとめ契約なし
										para.PrintString(g, offset, PrintData.AgreeMonthes.ToString());
									}
								}
							}
						}
						else if ("<金額" == StringUtil.Left(para.Entry, 3))
						{
							...
							if (null != est)
							{
								if (false == est.ChildServide)
								{
									...
									{
										// まとめ契約なし
										para.PrintString(g, offset, StringUtil.CommaEdit(est.Price * PrintData.AgreeMonthes));
									}
								}
							}
						}
						else if ("<有効期限>" == para.Entry)
						{
							...
							{
								para.PrintString(g, offset, PrintData.LimitDate.GetNormalString());
							}
						}

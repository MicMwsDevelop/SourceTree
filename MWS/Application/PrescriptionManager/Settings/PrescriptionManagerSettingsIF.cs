//
// PrescriptionManagerSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace PrescriptionManager.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class PrescriptionManagerSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "PrescriptionManagerSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static PrescriptionManagerSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(PrescriptionManagerSettings));
						Settings = serializer.Deserialize(fileStream) as PrescriptionManagerSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new PrescriptionManagerSettings();
				}
			}
			return result;
		}
		...
		public static PrescriptionManagerSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as PrescriptionManagerSettings;
		}
		...
		public static void SetSettings(PrescriptionManagerSettings settings)
		{
			SetSettingsFileName();

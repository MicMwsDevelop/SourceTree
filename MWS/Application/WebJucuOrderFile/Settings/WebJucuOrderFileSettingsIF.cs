...
namespace WebJucuOrderFile.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class WebJucuOrderFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "WebJucuOrderFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static WebJucuOrderFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(WebJucuOrderFileSettings));
						Settings = serializer.Deserialize(fileStream) as WebJucuOrderFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new WebJucuOrderFileSettings();
				}
			}
			return result;
		}
		...
		public static WebJucuOrderFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as WebJucuOrderFileSettings;
		}
		...
		public static void SetSettings(WebJucuOrderFileSettings settings)
		{
			SetSettingsFileName();

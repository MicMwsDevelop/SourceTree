//
// SetApplicationInfoSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace SetApplicationInfo.Settings
{
	/// <summary>
	/// 環境設定インターフェイス
	/// </summary>
	public static class SetApplicationInfoSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "SetApplicationInfoSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static SetApplicationInfoSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(SetApplicationInfoSettings));
						Settings = serializer.Deserialize(fileStream) as SetApplicationInfoSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new SetApplicationInfoSettings();
				}
			}
			return result;
		}
		...
		public static SetApplicationInfoSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as SetApplicationInfoSettings;
		}
		...
		public static void SetSettings(SetApplicationInfoSettings settings)
		{
			SetSettingsFileName();

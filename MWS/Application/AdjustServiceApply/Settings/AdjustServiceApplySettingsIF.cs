//
// AdjustServiceApplySettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace AdjustServiceApply.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class AdjustServiceApplySettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "AdjustServiceApplySettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static AdjustServiceApplySettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(AdjustServiceApplySettings));
						Settings = serializer.Deserialize(fileStream) as AdjustServiceApplySettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new AdjustServiceApplySettings();
				}
			}
			return result;
		}
		...
		public static AdjustServiceApplySettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as AdjustServiceApplySettings;
		}
		...
		public static void SetSettings(AdjustServiceApplySettings settings)
		{
			SetSettingsFileName();

//
// CloudDataBankStockDataOutputSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace CloudDataBankStockDataOutput
{
	...
	/// </summary>
	public static class CloudDataBankStockDataOutputSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "CloudDataBankStockDataOutputSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static CloudDataBankStockDataOutputSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(CloudDataBankStockDataOutputSettings));
						Settings = serializer.Deserialize(fileStream) as CloudDataBankStockDataOutputSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new CloudDataBankStockDataOutputSettings();
				}
			}
			return result;
		}
		...
		public static CloudDataBankStockDataOutputSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as CloudDataBankStockDataOutputSettings;
		}
		...
		public static void SetSettings(CloudDataBankStockDataOutputSettings settings)
		{
			SetSettingsFileName();

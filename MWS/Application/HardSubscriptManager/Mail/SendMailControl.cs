...
using HardSubscManager.Properties;

namespace HardSubscriptManager.Mail
{
	/// <summary>
	/// メール送信クラス
	/// </summary>
	public static class SendMailControl
	{
		...
		public static void SendMailToGyomu(Date useEndDate, List<HardSubscriptNotify> notifyList)
		{
			using (MailMessage msg = new MailMessage())
			{
				...
				string ymStr = string.Format("{0}年{1}月", useEndDate.Year, useEndDate.Month);
				...
				if (null != notifyList)
				{
					msg.Body += @"<table style=""BORDER-COLLAPSE: collapse"" bordercolor=""black"" border=1>"
								...
								+ @"</tr>";
					foreach (HardSubscriptNotify notify in notifyList)
					{
				...
				else
				{
					msg.Body += @"<table style=""BORDER-COLLAPSE: collapse"" bordercolor=""black"" border=1>"
								...
								+ @"</tr>"
								+ @"</table>";
					msg.Body += string.Format(@"<br><p>{0}にハードサブスク契約の契約満了を迎えるユーザーはいませんでした。</p>", ymStr);
				}
				msg.Body += @"</div>"
							+ @"<div>"
		...
		public static void SendMailToOffice(Date useEndDate, List<HardSubscriptNotify> notifyList, tMih支店情報 branch)
		{
			using (MailMessage msg = new MailMessage())
			{
				Encoding enc = Encoding.GetEncoding("shift_jis");

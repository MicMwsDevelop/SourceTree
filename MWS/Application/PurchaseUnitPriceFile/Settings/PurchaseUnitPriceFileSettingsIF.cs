//
// PurchaseUnitPriceFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace PurchaseUnitPriceFile.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class PurchaseUnitPriceFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "PurchaseUnitPriceFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static PurchaseUnitPriceFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(PurchaseUnitPriceFileSettings));
						Settings = serializer.Deserialize(fileStream) as PurchaseUnitPriceFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new PurchaseUnitPriceFileSettings();
				}
			}
			return result;
		}
		...
		public static PurchaseUnitPriceFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as PurchaseUnitPriceFileSettings;
		}
		...
		public static void SetSettings(PurchaseUnitPriceFileSettings settings)
		{
			SetSettingsFileName();

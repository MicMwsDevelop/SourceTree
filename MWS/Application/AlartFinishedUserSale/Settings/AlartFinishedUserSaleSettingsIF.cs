//
// AlartFinishedUserSaleSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace AlartFinishedUserSale.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class AlartFinishedUserSaleSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "AlartFinishedUserSaleSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static AlartFinishedUserSaleSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(AlartFinishedUserSaleSettings));
						Settings = serializer.Deserialize(fileStream) as AlartFinishedUserSaleSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new AlartFinishedUserSaleSettings();
				}
			}
			return result;
		}
		...
		public static AlartFinishedUserSaleSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as AlartFinishedUserSaleSettings;
		}
		...
		public static void SetSettings(AlartFinishedUserSaleSettings settings)
		{
			SetSettingsFileName();

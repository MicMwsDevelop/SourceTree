...
namespace OnlineElectricPrescriptEarningsFile.Log
{
	/// <summary>
	/// ログ出力クラス
	/// </summary>
	public static class ErrorLogOut
    {
		/// <summary>
		/// リトライ回数：20回 
		/// </summary>
		...
		/// エラーログファイルパス名
		/// </summary>
		private static string LogPathname = string.Empty;

		/// <summary>
		/// エラーログファイルパス名の設定
		/// </summary>
		/// <param name="folder">パス</param>
		/// <returns>エラーログファイルパス名</returns>
		public static string SetLogFileName(string folder)
		{
			string filename = string.Format("Error_{0}.log", DateTime.Now.ToString("yyyyMMddHH:mm:ss"));
			return LogPathname = Path.Combine(folder, filename);
		}

<%@ Page Language="VB" MasterPageFile="~/BusinessPlanningWeb.master" AutoEventWireup="false" CodeFile="OnlineDemand.aspx.vb" Inherits="OnlineDemand_OnlineDemand" title="オンライン請求作業申請済医院一覧" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderbody" Runat="Server">
	...
	<br />
	<asp:Label ID="LabelTitle" runat="server" Font-Bold="True" Font-Size="Large" Text="オンライン請求作業済申請医院一覧" />
	&nbsp;&nbsp;&nbsp;&nbsp;
	</p>
	<p>
	...
	<asp:DropDownList ID="comboBoxGoods" runat="server" AutoPostBack="True">
		<asp:ListItem Value="%">すべて</asp:ListItem>
		<asp:ListItem Value="044029">訪問</asp:ListItem>
		<asp:ListItem Value="044030">リモート</asp:ListItem>
	</asp:DropDownList>
	&nbsp;
	...
	<asp:GridView ID="GridViewOnlineDemand" runat="server" AutoGenerateColumns="False"
		...
		<Columns>
			<asp:BoundField DataField="営業部名" HeaderText="担当部署" SortExpression="担当部署" />
			...
			<asp:BoundField DataField="申請日時" DataFormatString="{0:yyyy/MM/dd}"
				HeaderText="申請日" SortExpression="申請日" />
			<asp:BoundField DataField="売上日時" DataFormatString="{0:yyyy/MM/dd}"
				HeaderText="売上日" SortExpression="売上日" />
	...
	<asp:SqlDataSource ID="SqlDataSourceOnlineDemand" runat="server"
		...
		SelectCommand="SELECT [営業部コード],[営業部名],[拠点コード],[拠点名],[顧客No],[顧客名],[得意先コード],[商品コード],[商品名],[売上金額],[受付No],[申請日時],[売上日時] FROM [vBPW_オンライン請求作業済申請情報] WHERE ([営業部名] LIKE '%' + @SC + '%') AND ([拠点名] LIKE '%' + @オフィス + '%') AND ([商品コード] LIKE '%' + @商品 + '%') ORDER BY [営業部コード],[拠点コード],[顧客No],[申請日時]">
		<SelectParameters>

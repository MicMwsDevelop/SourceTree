//
// OnlineDemandEarningsFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace OnlineDemandEarningsFile.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class OnlineDemandEarningsFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "OnlineDemandEarningsFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static OnlineDemandEarningsFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(OnlineDemandEarningsFileSettings));
						Settings = serializer.Deserialize(fileStream) as OnlineDemandEarningsFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new OnlineDemandEarningsFileSettings();
				}
			}
			return result;
		}
		...
		public static OnlineDemandEarningsFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as OnlineDemandEarningsFileSettings;
		}
		...
		public static void SetSettings(OnlineDemandEarningsFileSettings settings)
		{
			SetSettingsFileName();

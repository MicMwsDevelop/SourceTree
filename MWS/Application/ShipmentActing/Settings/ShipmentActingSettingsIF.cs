...
namespace ShipmentActing.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class ShipmentActingSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILE_NAME = " ShipmentActingSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static ShipmentActingSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(ShipmentActingSettings));
						Settings = serializer.Deserialize(fileStream) as ShipmentActingSettings;
					}
					catch (Exception)
					{
						result = false;
		...
		public static ShipmentActingSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as ShipmentActingSettings;
		}
		...
		public static void SetSettings(ShipmentActingSettings settings)
		{
			SetSettingsFileName();

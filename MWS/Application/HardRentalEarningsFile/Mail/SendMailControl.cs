...
namespace HardRentalEarningsFile.Mail
{
	/// <summary>
	/// メール送信クラス
	/// </summary>
	public static class SendMailControl
	{
		...
		/// メール送信（業務課宛て）
		/// </summary>
		/// <param name="saleList">ハードレンタル売上情報リスト</param>
		/// <param name="formalFilename">出力ファイル名</param>
		/// <param name="useDate">利用日</param>
		public static void SendMail(List<HardRentalEarningsOut> saleList, string formalFilename, Date useDate)
		{
			using (MailMessage msg = new MailMessage())
			{
				...
				// 件名
				msg.Subject = "ハードレンタル自動更新 売上連絡";

				// 本文
				msg.Body += string.Format(@"<div>"
							+ @"<p>業務管理部 業務課各位</p>"
							+ @"<p>ハードレンタルの課金終了日の更新と売上データを作成しました。<br>"
							+ @"<br>"
				...
				else
				{
					msg.Body += @"<table style=""BORDER-COLLAPSE: collapse"" bordercolor=""black"" border=1>"
								...
								+ @"</tr>"
								+ @"</table>";
					msg.Body += @"<br><p>ハードレンタル更新対象医院はありませんでした。</p>";
				}
				msg.Body += @"</div>"
							+ @"<div>"

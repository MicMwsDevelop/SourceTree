//
// HardRentalEarningsFileSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace HardRentalEarningsFile.Settings
{
	/// <summary>
	/// 環境設定インターフェイス
	/// </summary>
	public static class HardRentalEarningsFileSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "HardRentalEarningsFileSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static HardRentalEarningsFileSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(HardRentalEarningsFileSettings));
						Settings = serializer.Deserialize(fileStream) as HardRentalEarningsFileSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new HardRentalEarningsFileSettings();
				}
			}
			return result;
		}
		...
		public static HardRentalEarningsFileSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as HardRentalEarningsFileSettings;
		}
		...
		public static void SetSettings(HardRentalEarningsFileSettings settings)
		{
			SetSettingsFileName();

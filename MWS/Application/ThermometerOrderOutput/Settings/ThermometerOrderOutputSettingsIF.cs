//
// ThermometerOrderOutputSettingsIF.cs
// 
// 環境設定インターフェイス
// 
...
namespace ThermometerOrderOutput.Settings
{
	...
	/// 環境設定インターフェイス
	/// </summary>
	public static class ThermometerOrderOutputSettingsIF
	{
		/// <summary>
		/// 環境設定ファイル名称
		/// </summary>
		public const string SETTINGS_FILENAME = "ThermometerOrderOutputSettings.xml";

		/// <summary>
		/// 環境設定
		/// </summary>
		private static ThermometerOrderOutputSettings Settings = null;
			...
			if (reload || null == Settings)
			{
				...
				if (true == File.Exists(SettingsFileName))
				{
					...
					try
					{
						...
						XmlSerializer serializer = new XmlSerializer(typeof(ThermometerOrderOutputSettings));
						Settings = serializer.Deserialize(fileStream) as ThermometerOrderOutputSettings;
					}
					catch (Exception)
					{
						result = false;
				...
				{
					// 存在しない場合は初期値を設定
					Settings = new ThermometerOrderOutputSettings();
				}
			}
			return result;
		}
		...
		public static ThermometerOrderOutputSettings GetSettings(bool reload = false)
		{
			SetSettingsFileName();
			...
			return Settings.Clone() as ThermometerOrderOutputSettings;
		}
		...
		public static void SetSettings(ThermometerOrderOutputSettings settings)
		{
			SetSettingsFileName();

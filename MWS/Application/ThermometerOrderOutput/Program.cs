...
// Program.cs
// 
// 皮膚赤外線体温計受注明細出力 プログラムクラス
// 
// Copyright (C) MIC All Rights Reserved.
// 
// Ver1.00 新規作成(2020/09/23 勝呂)
//
using ThermometerOrderOutput.Forms;
...
namespace ThermometerOrderOutput
{
	static class Program
	{
		...
		/// プログラム名
		/// </summary>
		public const string PROC_NAME = "皮膚赤外線体温計受注明細出力";
		...
		static int Main()
		{
			...
			if (2 <= cmds.Length)
			{
				...
				{
					string msg = OutputCsvFile(ThermometerOrderOutputSettingsIF.GetSettings());
					if (0 < msg.Length)
					{
						return 1;
					}
		...
		/// <summary>
		/// 皮膚赤外線体温計受注データCSVファイルの出力
		/// </summary>
		/// <param name="settings">出力ファイルパス名</param>
		/// <returns>エラーメッセージ</returns>
		public static string OutputCsvFile(ThermometerOrderOutputSettings settings)
		{
			try
			{
